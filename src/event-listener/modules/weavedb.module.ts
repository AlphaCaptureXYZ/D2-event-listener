...
const getAllData = async <T>(
  ...
  try {
    const { byUserWalletFilter, type, wallet } = payload;
    ...
    while (done === false) {
      ...
      try {
        ...
        console.log(
          '[weavedb] getAllData (error "db.cget(...)")',
          err?.message
        );
      ...
      (await Promise?.all(
        docs?.map(async (res) => {
          try {
            ...
            const acConditions = accessControlConditions(
              chain,
              userWallet,
              pkpWalletAddress
            );
            ...
            const { encryptedData, encryptedSymmetricKey } = doc;
...
const addData = async <T>(
  ...
  try {
    ...
    let { jsonData, userWallet, type, pkpKey, isCompressed, additionalInfo } =
      payload;
    ...
    const { encryptedFile, encryptedSymmetricKeyString } =
    ...
    const tx = await db.add(obj, COLLECTION_NAME);
