...
CURRENT_ID=$(sudo docker ps -a -q --filter ancestor=d2-event-listener-app)
echo "the current id is $CURRENT_ID"
echo "Building the DII Site image"
sudo docker build . --no-cache -t d2-event-listener-app --file Dockerfile
...
sleep 1s
sudo docker run -d -p 3006:3006 d2-event-listener-app
